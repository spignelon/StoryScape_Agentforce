...
import uuid  # Added for generating unique session keys
...
class GenerateComic:
    def __init__(self, update_state=None, lang_code="en"):
        ...
        # Agentforce API configuration from environment variables
        self.AGENTFORCE_API_BASE = os.getenv("AGENTFORCE_API_BASE", "https://api.salesforce.com/einstein/ai-agent/v1")
        self.MY_DOMAIN_URL = os.getenv("MY_DOMAIN_URL")
        self.AGENT_ID = os.getenv("AGENT_ID")
        self.ACCESS_TOKEN = os.getenv("ACCESS_TOKEN")
        
        if not all([self.MY_DOMAIN_URL, self.AGENT_ID, self.ACCESS_TOKEN]):
            print("Warning: One or more Agentforce API credentials are missing from environment variables")
    ...
    def establish_agentforce_session(self):
        """Establish a session with the Agentforce API"""
        url = f"{self.AGENTFORCE_API_BASE}/agents/{self.AGENT_ID}/sessions"
        ...
        try:
            ...
            print(f"Error establishing Agentforce session: {e}")
            raise

    def send_message_to_session(self, session_id, message):
        """Send a message to an established session"""
        url = f"{self.AGENTFORCE_API_BASE}/sessions/{session_id}/messages/stream"
        ...
        try:
            ...
            print(f"Error sending message to Agentforce session: {e}")
            raise

    def gen_text(self, text):
        """Generate text using the Agentforce API"""
        try:
            # Establish a session with Agentforce
            session_response = self.establish_agentforce_session()
            session_id = session_response.get("sessionId")
            
            if not session_id:
                raise Exception("Failed to get session ID from Agentforce API")
            
            # Send message to the session
            message_response = self.send_message_to_session(session_id, text)
            
            # Extract the response text from the messages
            messages = message_response.get("messages", [])
            if not messages:
                raise Exception("No messages returned from Agentforce API")
            ...
            # Fallback to original API if Agentforce fails
